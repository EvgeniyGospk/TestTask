...
1) Paywall → “Попробовать бесплатно” → Meditations  
2) Meditations → открыть AI “Настрой дня” → выбрать настроение → “Сгенерировать” → увидеть текст  
3) Locked‑поведение проверять на “чистом” состоянии (без подписки):
