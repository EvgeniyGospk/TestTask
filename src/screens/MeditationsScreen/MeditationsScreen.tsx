import { Pressable, Text, View } from 'react-native';

import { StackScreenProps } from '@react-navigation/stack';

import { routes, RootStackParamList } from '../../app/navigation/routes';

type Props = StackScreenProps<RootStackParamList, typeof routes.Meditations>;

export function MeditationsScreen({ navigation }: Props) {
  return (
    <View className="flex-1 items-center justify-center bg-[#07070C] px-6">
      <Text className="text-3xl font-bold text-white">Meditations</Text>
      <Text className="mt-3 text-center text-sm text-white/70">
        Здесь будет список сессий и locked‑логика.
      </Text>

      <Pressable
        accessibilityRole="button"
        onPress={() => navigation.navigate(routes.Paywall)}
        className="mt-6 rounded-2xl border border-white/15 bg-white/5 px-5 py-4"
        style={({ pressed }) => [{ opacity: pressed ? 0.92 : 1 }]}
      >
        <Text className="text-sm font-semibold text-white/80">Открыть Paywall</Text>
      </Pressable>
    </View>
  );
}

